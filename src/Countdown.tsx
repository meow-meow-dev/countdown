...
function DurationUnit({ unitLabels, value }: DurationUnitProps): React.JSX.Element {
...
export function Countdown({ className, label, to }: CountdownProps): React.JSX.Element {
  ...
  return (
    <Stack className={className} align="center">
      <Title order={1} className="text-[5rem] text-center leading-none">{label}</Title>
      <Text className="text-3xl">Il reste</Text>
      <Flex direction={{ base: "column", sm: "row" }} align="center" className="text-5xl gap-3">
      ...
      <Text className="text-xl lg:text-3xl">
        Jusqu'au
        {" "}
        {to.toLocaleString(DateTime.DATE_FULL, { locale: "fr-FR" })}
      </Text>
    </Stack>
  )
}
