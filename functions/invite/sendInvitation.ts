...
export async function sendInvitation(params: SendEmailProps, env: Env): Promise<void> {
  const { SEND_INVITATION_FROM, WEBSITE_NAME, WEBSITE_URL } = env
  if (SEND_INVITATION_FROM === undefined || WEBSITE_NAME === undefined || WEBSITE_URL === undefined)
    return

  const templateParams = { ...params, website: { name: WEBSITE_NAME, url: WEBSITE_URL } }
  const body = (generateBody(templateParams))
  const subject = generateSubject(templateParams)

  await sendEmail({ body, from: SEND_INVITATION_FROM, subject, to: params.to.email }, env)
}
